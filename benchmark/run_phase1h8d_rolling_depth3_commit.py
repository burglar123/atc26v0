...
CASE_PRESETS: dict[str, RollingDepth3CommitCase] = {
    "baseline_8c_depth3_shadow": RollingDepth3CommitCase(
    ...
    "rolling_depth3_commit": RollingDepth3CommitCase(
    ...
    "rolling_depth3_pressure_commit": RollingDepth3CommitCase(
    ...
    "rolling_gamma2_depth3_commit": RollingDepth3CommitCase(
...
def run_case(args: argparse.Namespace, case: RollingDepth3CommitCase, env: dict[str, str]) -> tuple[int, dict[str, Any]]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1H-8d rolling depth-3 commit cases.")
    parser.add_argument("--draft-model")
    parser.add_argument("--target-model")
    parser.add_argument("--workload-in")
    parser.add_argument("--out-root", default="results/multislo/phase1h8d_rolling_depth3_commit")
    ...
    for case_name in select_cases(args.cases):
        ...
        if status != 0:
            ...
            if not args.keep_going:
                break
    summary_path = Path(args.out_root) / "phase1h8d_summary.json"
    if not args.print_only:

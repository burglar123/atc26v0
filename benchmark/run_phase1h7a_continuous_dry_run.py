...
CASE_PRESETS: dict[str, ContinuousCase] = {
    "baseline_one_shot": ContinuousCase(
        ...
        max_continuous_eager_requests_per_step=2,
        max_continuous_eager_tokens_per_step=8,
        max_continuous_eager_tokens_per_request=4,
        eager_trace_level="minimal",
        description="Existing guarded one-shot eager commit without continuous shadow.",
    ),
    "continuous_depth1": ContinuousCase(
        name="continuous_depth1",
        limit_requests=32,
        eval_batch_size=32,
        gamma=4,
        max_eager_requests_per_step=2,
        ...
        max_continuous_eager_tokens_per_request=4,
        eager_trace_level="minimal",
        description="Continuous eager shadow candidates one hop after one-shot commits.",
    ),
    "continuous_depth2": ContinuousCase(
        name="continuous_depth2",
        limit_requests=32,
        eval_batch_size=32,
        gamma=4,
        max_eager_requests_per_step=2,
        ...
        max_continuous_eager_tokens_per_request=4,
        eager_trace_level="minimal",
        description="Depth-2 shadow chain bookkeeping; depth-2 remains not-ready until real continuous exists.",
    ),
    "continuous_gamma2_depth2": ContinuousCase(
        name="continuous_gamma2_depth2",
        limit_requests=32,
        eval_batch_size=32,
        gamma=2,
        max_eager_requests_per_step=2,
        ...
        max_continuous_eager_chain_depth=2,
        max_continuous_eager_requests_per_step=2,
        max_continuous_eager_tokens_per_step=4,
        max_continuous_eager_tokens_per_request=2,
        eager_trace_level="minimal",
        description="Gamma-2 continuous shadow bookkeeping.",
    ),
    "continuous_pressure_depth2": ContinuousCase(
        name="continuous_pressure_depth2",
        limit_requests=32,
        eval_batch_size=32,
        gamma=4,
        ...
        continuous_enabled=True,
        max_continuous_eager_chain_depth=2,
        max_continuous_eager_requests_per_step=4,
        max_continuous_eager_tokens_per_step=16,
        max_continuous_eager_tokens_per_request=4,
        eager_trace_level="summary",
        description="Moderate eager pressure plus depth-2 continuous shadow bookkeeping.",
    ),
}
...
def eval_command(args: argparse.Namespace, case: ContinuousCase, result_json: Path, engine_trace: Path) -> list[str]:
    command = [
        args.python,
    ...
    if case.continuous_enabled:
        command.extend(
            [
                "--enable-continuous-eager-dry-run",
                "--max-continuous-eager-chain-depth",
                str(case.max_continuous_eager_chain_depth),
                "--max-continuous-eager-requests-per-step",
                str(case.max_continuous_eager_requests_per_step),
                "--max-continuous-eager-tokens-per-step",
                str(case.max_continuous_eager_tokens_per_step),
                "--max-continuous-eager-tokens-per-request",
                str(case.max_continuous_eager_tokens_per_request),
            ]
        )
...
def load_case_summary(engine_trace: Path, result_json: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "continuous_commit_ready_shadow_tokens": accounting.get(
            "continuous_eager_commit_ready_shadow_token_count"
        ),
        "continuous_estimated_token_share": accounting.get(
            "continuous_eager_estimated_committed_token_share_of_output"
        ),
        "combined_estimated_token_share": accounting.get("combined_estimated_token_share_of_output"),
        "continuous_chain_length_distribution": accounting.get("continuous_eager_chain_length_distribution"),
        "continuous_drop_reason_counts": accounting.get("continuous_eager_drop_reason_counts"),
        "continuous_duplicate_count": accounting.get("continuous_eager_duplicate_count"),
        "continuous_frontier_mismatch_count": accounting.get("continuous_eager_frontier_mismatch_count"),
        "continuous_real_commit_count": accounting.get("continuous_eager_real_commit_count"),
        "payload_units_per_ready_token": accounting.get("continuous_eager_payload_len_units_per_ready_token", 0.0),
...
def run_case(args: argparse.Namespace, case: ContinuousCase, env: dict[str, str]) -> tuple[int, dict[str, Any]]:
    ...
    if status == 0:
        for checker, input_kind in BASE_CHECKERS:
            ...
            if checker_status != 0:
                ...
                break
    if status == 0 and case.continuous_enabled:
        checker_status = run_command(
            checker_command(args, "benchmark/check_continuous_eager_dry_run.py", engine_trace),
            env,
            args.print_only,
        )
        if checker_status != 0:
            status = checker_status
            check_status = "failed:benchmark/check_continuous_eager_dry_run.py"
    row: dict[str, Any] = {
        "case_name": case.name,
        "continuous_enabled": case.continuous_enabled,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1H-7a continuous eager shadow dry-run cases.")
    parser.add_argument("--draft-model")
    parser.add_argument("--target-model")
    parser.add_argument("--workload-in")
    parser.add_argument("--out-root", default="results/multislo/phase1h7a_continuous_dry_run")
    parser.add_argument("--cases", default="baseline_one_shot,continuous_depth1")
...
def main() -> int:
    ...
    if not args.print_only:
        summary_path = Path(args.out_root) / "phase1h7a_summary.json"
        summary_path.parent.mkdir(parents=True, exist_ok=True)
        with summary_path.open("w", encoding="utf-8") as f:
            json.dump({"cases": rows}, f, indent=2, ensure_ascii=False)
        print(f"\n[OK] Wrote Phase 1H-7a summary: {summary_path}")

    print("\nPhase 1H-7a continuous shadow summary:")
    for row in rows:
        print(
            f"- {row['case_name']}: {row['check_status']} "
            f"one_shot_tokens={row.get('one_shot_committed_tokens', 'n/a')} "
            f"continuous_candidate_tokens={row.get('continuous_candidate_tokens', 'n/a')} "
            f"continuous_ready_shadow_tokens={row.get('continuous_commit_ready_shadow_tokens', 'n/a')} "
            f"combined_share={row.get('combined_estimated_token_share', 'n/a')}"
        )
    return status

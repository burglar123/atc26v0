...
def group_profile(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    target_home_size = max([int(r.get("target_home_size") or 0) for r in records], default=0)
    draft_home_size = max([int(r.get("draft_home_size") or 0) for r in records], default=0)
...
def dual_bottleneck(row: dict[str, Any]) -> str:
    ...
    priming_time_pct = row["phase_time"].get("priming", 0.0) / total
    if row["mean_overlap_ratio"] < 0.2:
        return "low overlap: dual-batch is likely serialized in practice"
    if fallback_time_pct > 0.2:
        return "high fallback time"
    if row["proposal_buffer_hit_rate"] < 0.8:
        return "proposal buffer miss rate is high"
    if row["draft_waste_rate"] > 0.5:
        return "draft waste is high"
    if row["mean_target_home_size"] and row["mean_draft_home_size"] and row["mean_target_home_size"] < 0.4 * (row["mean_target_home_size"] + row["mean_draft_home_size"]):
        return "target batch splitting may reduce hardware utilization"
    if priming_time_pct > 0.2:
        return "high priming time"

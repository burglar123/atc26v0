...
CASE_PRESETS: dict[str, BoundedRegressionCase] = {
    "baseline_8c_depth3_shadow": BoundedRegressionCase(
        "baseline_8c_depth3_shadow", 32, 32, 4, 2, 8, 4, 1, False, 2, 8, 4, 3, 2, 2, "minimal",
        "Depth-3 shadow baseline with guarded depth-3 commit disabled.",
    ),
    "rolling_depth3_commit": BoundedRegressionCase(
        "rolling_depth3_commit", 32, 32, 4, 2, 8, 4, 1, True, 2, 8, 4, 3, 2, 2, "minimal",
        "Guarded rolling depth-3 real commit from ready shadow children.",
    ),
    "rolling_depth3_pressure_commit": BoundedRegressionCase(
        "rolling_depth3_pressure_commit", 32, 32, 4, 4, 16, 4, 1, True, 4, 16, 4, 3, 4, 4, "summary",
        "Moderate eager pressure with guarded rolling depth-3 real commit.",
    ),
    "rolling_gamma2_depth3_commit": BoundedRegressionCase(
        "rolling_gamma2_depth3_commit", 32, 32, 2, 2, 4, 2, 1, True, 2, 4, 2, 3, 2, 2, "minimal",
        "Gamma-2 guarded rolling depth-3 real commit.",
    ),
    "rolling_depth3_commit_trace_minimal": BoundedRegressionCase(
        "rolling_depth3_commit_trace_minimal", 32, 32, 4, 2, 8, 4, 1, True, 2, 8, 4, 3, 2, 2, "minimal",
        "Guarded depth-3 commit with minimal trace output.",
    ),
    "rolling_depth3_commit_limit64": BoundedRegressionCase(
        "rolling_depth3_commit_limit64", 64, 32, 4, 2, 8, 4, 1, True, 2, 8, 4, 3, 2, 2, "minimal",
        "Guarded depth-3 commit with a 64-request limit.",
    ),
    "rolling_depth3_commit_batch16": BoundedRegressionCase(
        "rolling_depth3_commit_batch16", 32, 16, 4, 2, 8, 4, 1, True, 2, 8, 4, 3, 2, 2, "minimal",
        "Guarded depth-3 commit with eval batch size 16.",
    ),
    "rolling_depth3_commit_warmup_off": BoundedRegressionCase(
        "rolling_depth3_commit_warmup_off", 32, 32, 4, 2, 8, 4, 0, True, 2, 8, 4, 3, 2, 2, "minimal",
        "Guarded depth-3 commit with warmup disabled.",
    ),
}
...
def eval_command(
    ...
    if case.depth3_commit_enabled:
        command.append("--enable-rolling-continuous-depth3-commit-ready-only")
...
def load_case_summary(engine_trace: Path, result_json: Path) -> dict[str, Any]:
    ...
    summary = {
        ...
        "one_shot_committed_proposal_count": audit.get("one_shot_committed_proposal_count"),
        "depth1_committed_proposal_count": audit.get("depth1_committed_proposal_count"),
        "depth2_committed_proposal_count": audit.get("depth2_committed_proposal_count"),
        "depth3_committed_proposal_count": audit.get("depth3_committed_proposal_count"),
        ...
        "depth_gt3_committed_proposal_count": audit.get("depth_gt3_committed_proposal_count"),
        "normal_lane_conflict_count": audit.get("normal_lane_conflict_count"),
        "missing_buffered_proposal_unexpected_count": audit.get("missing_buffered_proposal_unexpected_count"),
        "duplicate_commit_count": audit.get("duplicate_commit_count"),
        "invalid_committed_child_count": audit.get("invalid_committed_child_count"),
        "cascade_committed_child_count": audit.get("cascade_committed_child_count"),
        "parent_missing_committed_child_count": audit.get("parent_missing_committed_child_count"),
        ...
        "committed_tokens_per_candidate_token": accounting.get("committed_tokens_per_candidate_token"),
        "committed_tokens_per_ready_token": accounting.get("committed_tokens_per_ready_token"),
        "total_eager_overhead_time_ms": accounting.get("total_eager_overhead_time_ms"),
        "continuous_eager_overhead_time_ms": accounting.get("continuous_eager_overhead_time_ms"),
        "rolling_depth2_commit_time_ms": accounting.get("rolling_depth2_commit_time_ms"),
        "rolling_depth3_shadow_generation_time_ms": accounting.get("rolling_depth3_shadow_generation_time_ms"),
        "rolling_depth3_commit_time_ms": accounting.get("rolling_depth3_commit_time_ms"),
        "performance_warnings": accounting.get("performance_warnings"),
    }
    return summary


def run_case(args: argparse.Namespace, case: BoundedRegressionCase, env: dict[str, str]) -> tuple[int, dict[str, Any]]:
    ...
    row: dict[str, Any] = {
        ...
        "depth3_commit_enabled": case.depth3_commit_enabled,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1H-8e bounded N=3 readiness regression cases.")
    parser.add_argument("--draft-model")
    parser.add_argument("--target-model")
    parser.add_argument("--workload-in")
    parser.add_argument("--out-root", default="results/multislo/phase1h8e_bounded_regression")
    parser.add_argument("--cases", default="baseline_8c_depth3_shadow,rolling_depth3_commit")
    ...
    for case_name in select_cases(args.cases):
        ...
        if status != 0:
            ...
            if not args.keep_going:
                break
    summary_path = Path(args.out_root) / "phase1h8e_summary.json"
    if not args.print_only:

...
CASE_PRESETS: dict[str, ContinuousCase] = {
    "baseline_one_shot": ContinuousCase(
        "baseline_one_shot", 32, 32, 4, 2, 8, 4, 1, False, 1, 2, 8, 4, "minimal",
        "Existing guarded one-shot eager commit without continuous verify/apply.",
    ),
    "continuous_depth1_verify": ContinuousCase(
        "continuous_depth1_verify", 32, 32, 4, 2, 8, 4, 1, True, 1, 2, 8, 4, "minimal",
        "Depth-1 continuous shadow proposals with verify/apply/result/sync dry-run.",
    ),
    "continuous_depth2_verify": ContinuousCase(
        "continuous_depth2_verify", 32, 32, 4, 2, 8, 4, 1, True, 2, 2, 8, 4, "minimal",
        "Depth-1 verify/apply plus explicit depth-2 parent-shadow blocking.",
    ),
    "continuous_gamma2_depth2_verify": ContinuousCase(
        "continuous_gamma2_depth2_verify", 32, 32, 2, 2, 4, 2, 1, True, 2, 2, 4, 2, "minimal",
        "Gamma-2 continuous verify/apply shadow run.",
    ),
    "continuous_pressure_depth2_verify": ContinuousCase(
        "continuous_pressure_depth2_verify", 32, 32, 4, 4, 16, 4, 1, True, 2, 4, 16, 4, "summary",
        "Moderate eager pressure with continuous verify/apply shadow run.",
    ),
}
    ...
    if case.continuous_enabled:
        command.extend(
            [
                ...
                "--max-continuous-eager-chain-depth", str(case.max_continuous_eager_chain_depth),
...
def load_case_summary(engine_trace: Path, result_json: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "continuous_full_accept_tokens": accounting.get("continuous_eager_full_accept_token_count"),
        ...
        "continuous_parent_shadow_not_committed_count": accounting.get(
            "continuous_parent_shadow_not_committed_count"
        ),
        "continuous_true_frontier_mismatch_count": accounting.get("continuous_true_frontier_mismatch_count"),
        "continuous_real_commit_count": accounting.get("continuous_eager_real_commit_count"),
        "max_continuous_depth_configured": max(
            [int(record.get("max_continuous_depth_configured") or 0) for record in records] or [0]
        ),
        "max_continuous_depth_observed": max(
            [int(record.get("max_continuous_depth_observed") or 0) for record in records] or [0]
        ),
        "continuous_shadow_stage": "verify_apply_dry_run" if any(
            record.get("continuous_eager_execution_stage") == "verify_apply_dry_run" for record in records
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1H-7b continuous eager verify/apply shadow cases.")
    parser.add_argument("--draft-model")
    parser.add_argument("--target-model")
    parser.add_argument("--workload-in")
    parser.add_argument("--out-root", default="results/multislo/phase1h7b_continuous_verify_apply")
    parser.add_argument("--cases", default="baseline_one_shot,continuous_depth1_verify")
...
def main() -> int:
    ...
    if not args.print_only:
        summary_path = Path(args.out_root) / "phase1h7b_summary.json"
        summary_path.parent.mkdir(parents=True, exist_ok=True)
        with summary_path.open("w", encoding="utf-8") as f:
            json.dump({"cases": rows}, f, indent=2, ensure_ascii=False)
        print(f"\n[OK] Wrote Phase 1H-7b summary: {summary_path}")

    print("\nPhase 1H-7b continuous verify/apply summary:")
    for row in rows:
        print(
            f"- {row['case_name']}: {row['check_status']} "
            f"one_shot_tokens={row.get('one_shot_committed_tokens', 'n/a')} "
            f"continuous_verified_tokens={row.get('continuous_verified_tokens', 'n/a')} "

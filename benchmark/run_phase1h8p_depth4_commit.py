...
CASE_PRESETS: dict[str, Depth4CommitCase] = {
    "baseline_8o_depth4_shadow": Depth4CommitCase(
        "baseline_8o_depth4_shadow", 32, 32, 4, 2, 8, 4, 1, True, False, 2, 8, 4, 4, 2, 2, "minimal",
        "Depth-4 shadow baseline with depth-4 real commit disabled.",
    ),
    "rolling_depth4_commit": Depth4CommitCase(
        "rolling_depth4_commit", 32, 32, 4, 2, 8, 4, 1, True, True, 2, 8, 4, 4, 2, 2, "minimal",
        "Guarded ready-only depth-4 real commit.",
    ),
    "rolling_depth4_pressure_commit": Depth4CommitCase(
        "rolling_depth4_pressure_commit", 32, 32, 4, 4, 16, 4, 1, True, True, 4, 16, 4, 4, 4, 4, "summary",
        "Moderate eager pressure with guarded ready-only depth-4 real commit.",
    ),
    "rolling_gamma2_depth4_commit": Depth4CommitCase(
        "rolling_gamma2_depth4_commit", 32, 32, 2, 2, 4, 2, 1, True, True, 2, 4, 2, 4, 2, 2, "minimal",
        "Gamma-2 guarded ready-only depth-4 real commit.",
    ),
}
...
CHECKERS = [
    ...
    ("benchmark/check_rolling_continuous_depth4_commit_ready_only.py", "trace_result", ()),
...
def eval_command(args: argparse.Namespace, case: Depth4CommitCase, result_json: Path, engine_trace: Path) -> list[str]:
    command = [
        args.python,
    ...
    if case.depth4_commit_enabled:
        command.append("--enable-rolling-continuous-depth4-commit-ready-only")
...
def load_case_summary(engine_trace: Path, result_json: Path, *, case_name: str, chain_summary_path: Path) -> dict[str, Any]:
    ...
    depth_token_parity_ok = all(
        ...
        for generic_key, legacy_key in (
            ...
            ("depth4_committed_token_count", "depth4_committed_token_count"),
        )
    )
    combined_parity_ok = (
    ...
    return {
        ...
        "rolling_depth4_commit_count": accounting.get("rolling_depth4_real_commit_count"),
        "rolling_depth4_commit_skip_reason_counts": accounting.get("rolling_depth4_real_commit_skip_reason_counts"),
        ...
        "rolling_depth4_commit_time_ms": accounting.get("rolling_depth4_commit_time_ms"),
        "rolling_depth4_commit_decision_broadcast_time_ms": accounting.get(
...
def run_case(args: argparse.Namespace, case: Depth4CommitCase, env: dict[str, str]) -> tuple[int, dict[str, Any]]:
    ...
    row: dict[str, Any] = {
        ...
        "depth4_commit_enabled": case.depth4_commit_enabled,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1H-8p rolling depth4 commit ready-only cases.")
    parser.add_argument("--draft-model")
    parser.add_argument("--target-model")
    parser.add_argument("--workload-in")
    parser.add_argument("--out-root", default="results/multislo/phase1h8p_depth4_commit")
    parser.add_argument("--cases", default="baseline_8o_depth4_shadow,rolling_depth4_commit")
    ...
    for case_name in select_cases(args.cases):
        ...
        if status != 0:
            ...
            if not args.keep_going:
                break
    summary_path = Path(args.out_root) / "phase1h8p_summary.json"
    if not args.print_only:

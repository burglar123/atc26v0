...
from benchmark.check_generic_rolling_apply_path_parity import build_summary as build_apply_summary  # noqa: E402
from benchmark.check_generic_rolling_runtime_parity import build_summary as build_runtime_summary  # noqa: E402
...
from benchmark.run_phase1h8s_generic_runtime_parity import (  # noqa: E402
    CASE_PRESETS as RUNTIME_CASE_PRESETS,
    GenericRuntimeParityCase,
    eval_command_8s,
    load_case_summary as load_8s_case_summary,
)
...
class GenericApplyPathParityCase:
    name: str
    runtime_case: GenericRuntimeParityCase
    generic_apply_enabled: bool
    description: str


CASE_PRESETS: dict[str, GenericApplyPathParityCase] = {
    "baseline_legacy_apply": GenericApplyPathParityCase(
        "baseline_legacy_apply",
        RUNTIME_CASE_PRESETS["baseline_8q_legacy_runtime"],
        False,
        "8t baseline with legacy depth-specific apply path and partial recovery disabled.",
    ),
    "generic_apply_baseline": GenericApplyPathParityCase(
        "generic_apply_baseline",
        RUNTIME_CASE_PRESETS["generic_runtime_baseline"],
        True,
        "8t generic rolling apply path with full-accept depth-4 baseline.",
    ),
    "partial_recovery_legacy_apply": GenericApplyPathParityCase(
        "partial_recovery_legacy_apply",
        RUNTIME_CASE_PRESETS["partial_recovery_legacy_runtime"],
        False,
        "8t partial-prefix recovery with legacy depth-specific apply path.",
    ),
    "generic_apply_partial_recovery": GenericApplyPathParityCase(
        "generic_apply_partial_recovery",
        RUNTIME_CASE_PRESETS["generic_runtime_partial_recovery"],
        True,
        "8t generic rolling apply path with partial-prefix recovery enabled.",
    ),
}
...
def eval_command_8t(
    ...
    case: GenericApplyPathParityCase,
    result_json: Path,
    engine_trace: Path,
) -> list[str]:
    command = eval_command_8s(args, case.runtime_case, result_json, engine_trace)
    if case.generic_apply_enabled:
        command.append("--enable-generic-rolling-apply-path")
        if "--enable-generic-rolling-runtime-loop" not in command:
...
def load_case_summary(
    ...
    *,
    ...
    generic_runtime = build_runtime_summary(records, result_payload)
    generic_apply = build_apply_summary(records, result_payload)
    row.update(
        {
            "generic_rolling_runtime_enabled": generic_runtime.get("generic_rolling_runtime_enabled"),
            "generic_rolling_apply_path_enabled": generic_apply.get("generic_rolling_apply_path_enabled"),
            "generic_rolling_apply_depths": generic_apply.get("generic_rolling_apply_depths"),
            "generic_rolling_apply_node_count": generic_apply.get("generic_rolling_apply_node_count"),
            "generic_rolling_apply_full_commit_token_count": generic_apply.get(
                "generic_rolling_apply_full_commit_token_count"
            ),
            "generic_rolling_apply_partial_recovered_token_count": generic_apply.get(
                "generic_rolling_apply_partial_recovered_token_count"
            ),
            "generic_rolling_apply_revised_token_count": generic_apply.get(
                "generic_rolling_apply_revised_token_count"
            ),
            "generic_rolling_apply_output_token_count": generic_apply.get(
                "generic_rolling_apply_output_token_count"
            ),
            "generic_rolling_apply_cascade_discard_count": generic_apply.get(
                "generic_rolling_apply_cascade_discard_count"
            ),
            "generic_rolling_apply_depth_gt4_count": generic_apply.get(
                "generic_rolling_apply_depth_gt4_count"
            ),
            "generic_rolling_apply_normal_lane_conflict_count": generic_apply.get(
                "generic_rolling_apply_normal_lane_conflict_count"
            ),
            "generic_rolling_apply_target_draft_mismatch_count": generic_apply.get(
                "generic_rolling_apply_target_draft_mismatch_count"
            ),
            "generic_rolling_apply_parity_ok": generic_apply.get("generic_rolling_apply_parity_ok"),
...
def run_case(
    ...
    status = run_command(eval_command_8t(args, case, result_json, engine_trace), env, args.print_only)
    ...
    partial_case = case.runtime_case.base_case
    depth4_case = partial_case.base_case
    row: dict[str, Any] = {
        "case_name": case.name,
        "depth2_commit_enabled": True,
        "depth3_shadow_enabled": True,
        "depth3_commit_enabled": True,
        "depth4_shadow_enabled": depth4_case.depth4_shadow_enabled,
        "depth4_commit_enabled": depth4_case.depth4_commit_enabled,
        "partial_prefix_recovery_enabled": partial_case.partial_recovery_enabled,
        "generic_rolling_runtime_enabled": case.runtime_case.generic_runtime_enabled,
        "generic_rolling_apply_path_enabled": case.generic_apply_enabled,
    ...
    if core_chain_passed and not args.print_only:
        ...
        if case.runtime_case.generic_runtime_enabled:
            parity_fields.append("generic_rolling_parity_ok")
        if case.generic_apply_enabled:
            parity_fields.append("generic_rolling_apply_parity_ok")
        parity_ok = all(bool(row.get(field)) for field in parity_fields)
        if not parity_ok:
            status = 1
            row["check_status"] = "failed:generic_apply_path_parity"
            row["core_checker_chain_status"] = "failed:generic_apply_path_parity"
            row["core_checker_chain_failed_checker"] = "generic_apply_path_parity"
    return status, row
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1H-8t generic rolling apply-path parity cases.")
    parser.add_argument("--draft-model")
    parser.add_argument("--target-model")
    parser.add_argument("--workload-in")
    parser.add_argument("--out-root", default="results/multislo/phase1h8t_generic_apply_path_parity")
    parser.add_argument(
        "--cases",
        default=(
            "baseline_legacy_apply,generic_apply_baseline,"
            "partial_recovery_legacy_apply,generic_apply_partial_recovery"
        ),
    )
    ...
    for case_name in select_cases(args.cases):
        ...
        if status != 0:
            ...
            if not args.keep_going:
                break
    summary_path = Path(args.out_root) / "phase1h8t_summary.json"
    if not args.print_only:

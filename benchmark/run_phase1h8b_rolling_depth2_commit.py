...
CASE_PRESETS: dict[str, RollingDepth2Case] = {
    "baseline_8a_shadow": RollingDepth2Case(
        "baseline_8a_shadow", 32, 32, 4, 2, 8, 4, 1, False, 2, 8, 4, 2, 2, 2, "minimal",
        "Rolling depth-2 shadow only, with no depth-2 real commit.",
    ),
    "rolling_depth2_commit": RollingDepth2Case(
        "rolling_depth2_commit", 32, 32, 4, 2, 8, 4, 1, True, 2, 8, 4, 2, 2, 2, "minimal",
        "Guarded real rolling depth-2 commit for ready shadow children.",
    ),
    "rolling_depth2_pressure_commit": RollingDepth2Case(
        "rolling_depth2_pressure_commit", 32, 32, 4, 4, 16, 4, 1, True, 4, 16, 4, 2, 4, 4, "summary",
        "Moderate eager pressure with guarded rolling depth-2 commit.",
    ),
    "rolling_gamma2_depth2_commit": RollingDepth2Case(
        "rolling_gamma2_depth2_commit", 32, 32, 2, 2, 4, 2, 1, True, 2, 4, 2, 2, 2, 2, "minimal",
        "Gamma-2 guarded rolling depth-2 commit.",
    ),
}
...
def eval_command(args: argparse.Namespace, case: RollingDepth2Case, result_json: Path, engine_trace: Path) -> list[str]:
    command = [
        args.python,
    ...
    if case.depth2_commit_enabled:
        command.append("--enable-rolling-continuous-depth2-commit-ready-only")
...
def load_case_summary(engine_trace: Path, result_json: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "rolling_depth2_real_commit_count": accounting.get("rolling_depth2_real_committed_proposal_count"),
        "rolling_depth_gt2_real_commit_count": accounting.get("rolling_depth_gt2_real_commit_count"),
        "rolling_depth2_commit_skip_reason_counts": accounting.get("rolling_depth2_real_commit_skip_reason_counts"),
        "rolling_normal_lane_conflict_count": accounting.get("rolling_normal_lane_conflict_count"),
        "rolling_parent_full_accept_count": accounting.get("rolling_parent_full_accept_count"),
        "rolling_parent_partial_reject_count": accounting.get("rolling_parent_partial_reject_count"),
        "rolling_parent_resolution_pending_count": accounting.get("rolling_parent_resolution_pending_count"),
        "rolling_drop_reason_counts": accounting.get("rolling_drop_reason_counts"),
    }


def run_case(args: argparse.Namespace, case: RollingDepth2Case, env: dict[str, str]) -> tuple[int, dict[str, Any]]:
    ...
    row: dict[str, Any] = {
        "case_name": case.name,
        "depth2_commit_enabled": case.depth2_commit_enabled,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 1H-8b rolling depth-2 commit cases.")
    parser.add_argument("--draft-model")
    parser.add_argument("--target-model")
    parser.add_argument("--workload-in")
    parser.add_argument("--out-root", default="results/multislo/phase1h8b_rolling_depth2_commit")
    parser.add_argument("--cases", default="baseline_8a_shadow,rolling_depth2_commit")
    ...
    for case_name in select_cases(args.cases):
        ...
        if status != 0:
            ...
            if not args.keep_going:
                break
    summary_path = Path(args.out_root) / "phase1h8b_summary.json"
    if not args.print_only:
